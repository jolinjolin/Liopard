...
e = np.array([1, 2, 3])
df = liop.DataFrame({'a': a, 'b': b, 'c': c, 'd': d, 'e': e})
...
class TestDataFrameCreation:
    ...
    def test_input_types(self):
        with pytest.raises(TypeError):
            liop.DataFrame([1, 2, 3])

        with pytest.raises(TypeError):
            liop.DataFrame({1: 5, 'b': 10})

        with pytest.raises(TypeError):
            liop.DataFrame({'a': np.array([1]), 'b': 10})

        with pytest.raises(ValueError):
            liop.DataFrame({'a': np.array([1]), 
                           'b': np.array([[1]])})
    ...
    def test_dtypes(self):
        ...
        df_result = df.dtypes
        df_answer = liop.DataFrame({'Column Name': cols,
                                   'Data Type': dtypes})
...
class TestSelection:
    ...
    def test_multiple_columns(self):
        cols = ['a', 'c']
        df_result = df[cols]
        df_answer = liop.DataFrame({'a': a, 'c': c})
        assert_df_equals(df_result, df_answer)

    def test_simple_boolean(self):
        bool_arr = np.array([True, False, False])
        df_bool = liop.DataFrame({'col': bool_arr})
        df_result = df[df_bool]
        df_answer = liop.DataFrame({'a': a[bool_arr], 'b': b[bool_arr], 
                                   'c': c[bool_arr], 'd': d[bool_arr], 
                                   'e': e[bool_arr]})
        assert_df_equals(df_result, df_answer)

        with pytest.raises(ValueError):
            df_bool = liop.DataFrame({'col': bool_arr, 'col2': bool_arr})
            df[df_bool]

        with pytest.raises(TypeError):
            df_bool = liop.DataFrame({'col': np.array[1, 2, 3]})

    def test_one_column_tuple(self):
        assert_df_equals(df[:, 'a'], liop.DataFrame({'a': a}))
    ...
    def test_int_selcetion(self):
        assert_df_equals(df[:, 3], liop.DataFrame({'d': d}))
    ...
    def test_single_element(self):
        df_answer = liop.DataFrame({'e': np.array([2])})
        assert_df_equals(df[1, 'e'], df_answer)

    def test_all_row_selections(self):
        df1 = liop.DataFrame({'a': np.array([True, False, True]),
                             'b': np.array([1, 3, 5])})
        with pytest.raises(ValueError):
            df[df1, 'e']
        ...
        df_answer = liop.DataFrame({'c': c[[True, False, True]]})
        assert_df_equals(df_result, df_answer)

        df_result = df[[1, 2], 0]
        df_answer = liop.DataFrame({'a': a[[1, 2]]})
        assert_df_equals(df_result, df_answer)
    ...
    def test_list_columns(self):
        df_answer = liop.DataFrame({'c': c, 'e': e})
        assert_df_equals(df[:, [2, 4]], df_answer)
        assert_df_equals(df[:, [2, 'e']], df_answer)
        assert_df_equals(df[:, ['c', 'e']], df_answer)

        df_result = df[2, ['a', 'e']]
        df_answer = liop.DataFrame({'a': a[[2]], 'e': e[[2]]})
        assert_df_equals(df_result, df_answer)

        df_answer = liop.DataFrame({'c': c[[1, 2]], 'e': e[[1, 2]]})
        assert_df_equals(df[[1, 2], ['c', 'e']], df_answer)

        df1 = liop.DataFrame({'a': np.array([True, False, True]),
                             'b': np.array([1, 3, 5])})
        df_answer = liop.DataFrame({'c': c[[0, 2]], 'e': e[[0, 2]]})
        assert_df_equals(df[df1['a'], ['c', 'e']], df_answer)

    def test_col_slice(self):
        df_answer = liop.DataFrame({'a': a, 'b': b, 'c': c})
        assert_df_equals(df[:, :3], df_answer)

        df_answer = liop.DataFrame({'a': a[::2], 'b': b[::2], 'c': c[::2]})
        assert_df_equals(df[::2, :3], df_answer)

        df_answer = liop.DataFrame({'a': a[::2], 'b': b[::2], 'c': c[::2], 'd': d[::2], 'e': e[::2]})
    ...
    def test_new_column(self):
        df_result = liop.DataFrame({'a': a, 'b': b, 'c': c, 'd': d, 'e': e})
        f = np.array([1.5, 23, 4.11])
        df_result['f'] = f
        df_answer = liop.DataFrame({'a': a, 'b': b, 'c': c, 'd': d, 'e': e, 'f': f})
        assert_df_equals(df_result, df_answer)
        ...
        f = np.array([1.5, 23, 4.11])
        df_result['c'] = f
        df_answer = liop.DataFrame({'a': a, 'b': b, 'c': f, 'd': d, 'e': e})
        assert_df_equals(df_result, df_answer)
        ...
        with pytest.raises(ValueError):
            df1 = liop.DataFrame({'a': np.random.rand(5)})
            df['a'] = df1
...
df1 = liop.DataFrame({'a': a1, 'b': b1, 'c': c1})

a2 = np.array([True, False])
b2 = np.array([True, True])
c2 = np.array([False, True])
df2 = liop.DataFrame({'a': a2, 'b': b2, 'c': c2})
...
class TestAggregation:
    ...
    def test_min(self):
        df_result = df1.min()
        df_answer = liop.DataFrame({'a': np.array(['a'], dtype='O'),
                                   'b': np.array([5]),
                                   'c': np.array([np.nan])})
        assert_df_equals(df_result, df_answer)
    ...
    def test_max(self):
        df_result = df1.max()
        df_answer = liop.DataFrame({'a': np.array(['c'], dtype='O'),
                                   'b': np.array([11]),
                                   'c': np.array([np.nan])})
        assert_df_equals(df_result, df_answer)
    ...
    def test_mean(self):
        df_result = df1.mean()
        df_answer = liop.DataFrame({'b': np.array([8.]),
                                   'c': np.array([np.nan])})
        assert_df_equals(df_result, df_answer)
    ...
    def test_sum(self):
        df_result = df1.sum()
        df_answer = liop.DataFrame({'a': np.array(['abc'], dtype='O'),
                                   'b': np.array([24]),
                                   'c': np.array([np.nan])})
        assert_df_equals(df_result, df_answer)
    ...
    def test_var(self):
        df_result = df1.var()
        df_answer = liop.DataFrame({'b': np.array([b1.var()]),
                                   'c': np.array([np.nan])})
        assert_df_equals(df_result, df_answer)

    def test_std(self):
        df_result = df1.std()
        df_answer = liop.DataFrame({'b': np.array([b1.std()]),
                                   'c': np.array([np.nan])})
        assert_df_equals(df_result, df_answer)

    def test_all(self):
        df_result = df2.all()
        df_answer = liop.DataFrame({'a': np.array([False]),
                                   'b': np.array([True]),
                                   'c': np.array([False])})
        assert_df_equals(df_result, df_answer)
    ...
    def test_any(self):
        df_result = df2.any()
        df_answer = liop.DataFrame({'a': np.array([True]),
                                   'b': np.array([True]),
                                   'c': np.array([True])})
        assert_df_equals(df_result, df_answer)
    ...
    def test_argmax(self):
        ...
        df_answer = liop.DataFrame({'a': np.array([2]),
                                   'b': np.array([0]),
                                   'c': np.array([1])})
        assert_df_equals(df_result, df_answer)
    ...
    def test_argmin(self):
        ...
        df_answer = liop.DataFrame({'a': np.array([0]),
                                   'b': np.array([1]),
                                   'c': np.array([1])})
        assert_df_equals(df_result, df_answer)
...
df3 = liop.DataFrame({'a': a3, 'b': b3, 'c': c3})

a4 = np.array(['a', 'a', 'c'], dtype='O')
b4 = np.array([11, 5, 5])
c4 = np.array([3.4, np.nan, 3.4])
df4 = liop.DataFrame({'a': a4, 'b': b4, 'c': c4})
...
class TestOtherMethods:
    ...
    def test_isna(self):
        df_result = df3.isna()
        df_answer = liop.DataFrame({'a': np.array([False, True, False]),
    ...
    def test_rename(self):
        ...
        df_answer = liop.DataFrame({'A': a4, 'b': b4, 'C': c4})
        assert_df_equals(df_result, df_answer)

    def test_drop(self):
        df_result = df4.drop(['a', 'b'])
        df_answer = liop.DataFrame({'c': c4})
        assert_df_equals(df_result, df_answer)


a42 = np.array([-11, 5, 3])
b42 = np.array([3.4, 5.1, -6])
df42 = liop.DataFrame({'a': a42, 'b': b42})
...
class TestNonAgg:
    ...
    def test_abs(self):
        df_result = df42.abs()
        df_answer = liop.DataFrame({'a': np.abs(a42), 'b': np.abs(b42)})
        assert_df_equals(df_result, df_answer)

    def test_cummin(self):
        df_result = df42.cummin()
        df_answer = liop.DataFrame({'a': np.array([-11, -11, -11]),
                                   'b': np.array([3.4, 3.4, -6])})
        assert_df_equals(df_result, df_answer)

    def test_cummax(self):
        df_result = df42.cummax()
        df_answer = liop.DataFrame({'a': np.array([-11, 5, 5]),
                                   'b': np.array([3.4, 5.1, 5.1])})
        assert_df_equals(df_result, df_answer)

    def test_cumsum(self):
        df_result = df42.cumsum()
        df_answer = liop.DataFrame({'a': np.array([-11, -6, -3]),
                                   'b': np.array([3.4, 8.5, 2.5])})
        assert_df_equals(df_result, df_answer)

    def test_clip(self):
        df_result = df42.clip(0, 4)
        df_answer = liop.DataFrame({'a': np.array([0, 4, 3]),
                                   'b': np.array([3.4, 4, 0])})
        assert_df_equals(df_result, df_answer)

    def test_round(self):
        df_result = df42.round(0)
        df_answer = liop.DataFrame({'a': np.array([-11, 5, 3]),
                                   'b': np.array([3, 5, -6])})
        assert_df_equals(df_result, df_answer)
    ...
    def test_diff(self):
        ...
        df_answer = liop.DataFrame({'a': np.array([np.nan, 16, -2]),
                                   'b': np.array([np.nan, 1.7, -11.1])})
        assert_df_equals(df_result, df_answer)

    def test_pct_change(self):
        df_result = df42.pct_change(1)
        df_answer = liop.DataFrame({'a': np.array([np.nan, 16 / -11, -2 / 5]),
...
class TestOperators:
    ...
    def test_add(self):
        df_result = df5 + 3
        df_answer = liop.DataFrame({'a': a5 + 3, 'b': b5 + 3})
        assert_df_equals(df_result, df_answer)
    ...
    def test_sub(self):
        ...
        df_result = 3 - df5
        df_answer = liop.DataFrame({'a': 3 - a5, 'b': 3 - b5})
        assert_df_equals(df_result, df_answer)
    ...
    def test_pow(self):
        ...
        df_result = 2 ** df5
        df_answer = liop.DataFrame({'a': 2 ** a5, 'b': 2 ** b5})
        assert_df_equals(df_result, df_answer)
    ...
    def test_gt_lt(self):
        ...
        df_result = df5 < 2
        df_answer = liop.DataFrame({'a': a5 < 2, 'b': b5 < 2})
        assert_df_equals(df_result, df_answer)
...
df6 = liop.DataFrame({'a': a6, 'b': b6})

a7 = np.array(['b', 'a', 'a', 'a', 'b'])
b7 = np.array([3.4, 5.1, 2, 1, 6])
df7 = liop.DataFrame({'a': a7, 'b': b7})
...
class TestMoreMethods:
    ...
    def test_sort_values(self):
        ...
        df_answer = liop.DataFrame({'a': a, 'b': b})
        assert_df_equals(df_result, df_answer)
    ...
    def test_sort_values_two_desc(self):
        ...
        a = np.array(['a', 'a', 'a', 'b', 'b'])
        b = np.array([1, 2, 5.1, 3.4, 6])
        df_answer = liop.DataFrame({'a': a[::-1], 'b': b[::-1]})
        assert_df_equals(df_result, df_answer)

    def test_sample(self):
        df_result = df7.sample(2, seed=1)
        df_answer = liop.DataFrame({'a': np.array(['a', 'a'], dtype=object),
                                   'b': np.array([2., 5.1])})
        assert_df_equals(df_result, df_answer)

        df_result = df7.sample(frac=.7, seed=1)
        df_answer = liop.DataFrame({'a': np.array(['a', 'a', 'b'], dtype=object),
                                   'b': np.array([2., 5.1, 6.])})
        assert_df_equals(df_result, df_answer)
...
df8 = liop.DataFrame({'a': a8, 'b': b8, 'c': c8})


class TestGrouping:

    def test_value_counts(self):
        df_temp = liop.DataFrame({'state': np.array(['texas', 'texas', 'texas', 'florida', 'florida', 'florida', 'florida', 'ohio']),
                                 'fruit': np.array(['a', 'a', 'a', 'a', 'b', 'b', 'b', 'a'])})
        df_results = df_temp.value_counts()
        df_answer = liop.DataFrame({'state': np.array(['florida', 'texas', 'ohio'], dtype=object),
                                   'count': np.array([4, 3, 1])})
        assert_df_equals(df_results[0], df_answer)

        df_answer = liop.DataFrame({'fruit': np.array(['a', 'b'], dtype=object),
                                   'count': np.array([5, 3])})
    ...
    def test_pivot_table_rows_or_cols(self):
        ...
        df_answer = liop.DataFrame({'a': np.array(['a', 'b'], dtype=object),
                                   'size': np.array([5, 3])})
        assert_df_equals(df_result, df_answer)
        ...
        df_answer = liop.DataFrame({'A': np.array([4]),
                                   'B': np.array([4])})
        assert_df_equals(df_result, df_answer)

        df_result = df8.pivot_table(columns='a', values='c', aggfunc='sum')
        df_answer = liop.DataFrame({'a': np.array([22]), 'b': np.array([14])})
        assert_df_equals(df_result, df_answer)
...
df_string = liop.DataFrame({'movie': movie, 'num': num})
...
class TestStrings:
    ...
    def test_capitalize(self):
        ...
        movie = np.array(['Field of dreams', 'Star wars'], dtype='O')
        answer = liop.DataFrame({'movie': movie})
...
df_emp = liop.read_csv('data/employee.csv')
